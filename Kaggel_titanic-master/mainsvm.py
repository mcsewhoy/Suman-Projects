...
# 1) SVM WITH NO SCALING

# SVM Marcus
print("\nSVM")
print("\nStarting SVM Single Default Run, No Scaling")
...
print("Starting SVM Gridsearch Run, no scaling")
Cs = [0.001, 0.01, 0.1, 1, 10]
gammas = [0.001, 0.01, 0.1, 1, 10]
kernels =['rbf','linear']
param_grid = {'C': Cs,'gamma':gammas,'kernel':kernels}
svm_gs = GridSearchCV(svm.SVC(),param_grid,cv=5, scoring='accuracy')
best_model_svm = svm_gs.fit(X_train, y_train)
print(best_model_svm.best_params_)
print(best_model_svm.best_score_)

print("Running test data with optimal Gridsearch Params, no scaling")
...
# SVM Marcus
print("Starting SVM Single Default Settings Run, Standard Scaling")
from sklearn import svm
clf = svm.SVC()
clf.fit(X_train_sc, y_train_sc)
pred_svm = clf.predict(X_test_sc)
print(accuracy_score(y_test_sc, pred_svm))


# Setting Grid Parameters
print("\nStarting SVM Gridsearch Run using Standard Scaler")
Cs = [0.001, 0.01, 0.1, 1, 10]
gammas = [0.001, 0.01, 0.1, 1, 10]
kernels =['rbf','linear']
param_grid = {'C': Cs,'gamma':gammas,'kernel':kernels}
svm_gs = GridSearchCV(svm.SVC(),param_grid,cv=5, scoring='accuracy')
best_model_svm = svm_gs.fit(X_train_sc, y_train_sc)
print(best_model_svm.best_params_)
print(best_model_svm.best_score_)

print("Running test data with optimal Gridsearch Params using Standard Scalar")
...
# SVM Marcus
print("\nStarting SVM Single Default Settings Run, MinMax Scaling")
from sklearn import svm
clf = svm.SVC()
clf.fit(X_train_sc1, y_train_sc1)
pred_svm = clf.predict(X_test_sc1)
print(accuracy_score(y_test_sc1, pred_svm))


# Setting Grid Parameters
print("Starting SVM Gridsearch Run using MinMAx Scaler")
Cs = [0.001, 0.01, 0.1, 1, 10]
gammas = [0.001, 0.01, 0.1, 1, 10]
kernels =['rbf','linear']
param_grid = {'C': Cs,'gamma':gammas,'kernel':kernels}
svm_gs = GridSearchCV(svm.SVC(),param_grid,cv=5, scoring='accuracy')
best_model_svm = svm_gs.fit(X_train_sc1, y_train_sc1)
print(best_model_svm.best_params_)
print(best_model_svm.best_score_)

print("Running test data with optimal Gridsearch Params using Min Max Scalar")
